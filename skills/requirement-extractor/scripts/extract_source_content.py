#!/usr/bin/env python3
"""Read-only source extraction for requirement scope analysis.

The script emits JSON Lines. It preserves format-specific locators but does not
decide whether a statement belongs to the declared target subject. Visual review
remains mandatory for layout-sensitive, diagram-heavy, merged, scanned, or
otherwise low-text files.

Domain specifics (the target subject's names/aliases and its boundary systems)
are supplied through an optional ``--config`` JSON file so the same helper works
across projects. Without a config the script still extracts every source and
applies only domain-agnostic triage hints (vague language, generic actor,
interface, exception, data-ownership, and acceptance-gap signals).
"""
...
# Domain-agnostic triage signals. These do not depend on the target subject.
VAGUE_SCOPE_PATTERN = re.compile(
    r"支持|具备|实现|提供|相关|等功能|智能(?:化)?|自动(?:化)?|优化|灵活|高效|稳定|完善|无缝|兼容|适配|及时|按需|必要时|可配置"
)
GENERIC_ACTOR_PATTERN = re.compile(
    r"(?:^|[，。；：\s])(系统|平台|软件|服务|模块|子系统|上位系统|控制端)(?:应|需|可|支持|负责|实现|提供)"
)
...
DATA_OWNERSHIP_PATTERN = re.compile(r"主数据|基础数据|状态同步|数据同步|缓存|回写|数据源|数据权属|档案|用户权限")
ACCEPTANCE_GAP_PATTERN = re.compile(r"实时|高效|稳定|灵活|兼容|无缝|自动|智能|可靠|快速|友好")

# Generic boundary hints applied when the config supplies no boundary_terms.
DEFAULT_BOUNDARY_TERMS = ["上游系统", "下游系统", "相邻系统", "外部系统", "对端系统", "第三方系统", "周边系统"]
...
def compile_terms_pattern(terms: Iterable[str]) -> re.Pattern | None:
    """Compile a list of terms into one case-insensitive pattern.

    Pure-ASCII alphanumeric terms get ``(?<![A-Za-z0-9])...(?![A-Za-z0-9])``
    lookarounds so short tokens like ``WMS`` do not match inside ``QWMS``.
    Non-ASCII (e.g. CJK) terms are matched as escaped substrings.
    """
    parts: list[str] = []
    for raw in terms:
        term = str(raw).strip()
...
def add_analysis_hints(
    record: dict[str, Any],
    anchor_pattern: re.Pattern | None,
    alias_pattern: re.Pattern | None,
    boundary_pattern: re.Pattern | None,
) -> dict[str, Any]:
    """Add deterministic triage hints without making a final scope classification."""
    ...
    explicit_anchor = bool(anchor_pattern and anchor_pattern.search(text))
    potential_alias = bool(alias_pattern and alias_pattern.search(text)) and not explicit_anchor
    signals: list[str] = []
    if boundary_pattern and boundary_pattern.search(text):
        signals.append("cross_system_boundary")
    if VAGUE_SCOPE_PATTERN.search(text):
        signals.append("vague_or_capability_language")
    if GENERIC_ACTOR_PATTERN.search(text) and not explicit_anchor:
    ...
    if explicit_anchor or potential_alias or signals:
        record["analysis_hints"] = {
            "explicit_anchor": explicit_anchor,
            "potential_alias": potential_alias,
            "priority_review_candidate": bool((explicit_anchor or potential_alias) and signals),
            "signals": signals,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract source text and stable locators as JSONL without modifying input files."
    )
    ...
    parser.add_argument(
        "--config",
        help="Optional JSON config with target_terms / alias_terms / boundary_terms (see domain-config.example.json)",
    )
...
def main() -> int:
    ...
    anchor_pattern = compile_terms_pattern(config.get("target_terms", []))
    alias_pattern = compile_terms_pattern(config.get("alias_terms", []))
    boundary_terms = config.get("boundary_terms") or DEFAULT_BOUNDARY_TERMS
    boundary_pattern = compile_terms_pattern(boundary_terms)
    ...
    try:
        for source_number, path in enumerate(files, start=1):
            ...
            for record in extract_file(path, source_id, args.max_chars):
                record = add_analysis_hints(record, anchor_pattern, alias_pattern, boundary_pattern)
