...
When the user does not specify a format, create `功能清单.md` with these sections:
...
| Field | Required | Rule |
|---|---:|---|
| `功能ID` | yes | Stable prefix: `FN`, `IF`, `NFR`, or `Q` (optionally namespaced, e.g. `支付-FN-001`) |
| `一级模块` | yes | Use a stable functional grouping; do not copy arbitrary document headings when they mix concerns |
| `二级功能` | yes | Concise behaviour name, normally verb + object |
| `功能说明` | yes | One atomic, testable behaviour; preserve source constraints |
| `触发/前置条件` | when known | Leave blank or `待澄清`; do not invent |
| `输入` | when known | Message/data/state/event |
...
| `相关系统/设备` | when known | Upstream, downstream, controller, or external actor |
| `分类` | yes | `主体功能`, `接口/协同`, `非功能要求`, `待澄清`, `主体外职责` |
| `证据强度` | yes | `明确`, `推断`, `歧义` |
| `置信度` | yes | `高`, `中`, `低` with notes for medium/low |
| `重点标注` | yes | `是/否`; set to `是` for vague or boundary-ambiguous passages |
...
Use these fields in `重点关注项`. A passage may appear here even when it cannot yet become a function row.

| `关注ID` | `关注级别` | `原文主题/忠实概述` | `模糊类型` | `可能责任方/解释` | `范围或实现影响` | `建议确认问题` | `关联功能ID` | `出处ID` | `状态` |
|---|---|---|---|---|---|---|---|---|---|

- `关注ID`: use `FOCUS-001` style IDs.
...
- `重点关注`: all vague or responsibility-ambiguous passages, sorted by `关注级别` and linked to provisional functions/questions
...
Report counts for analyzed files, partial/unreadable files, target-subject anchor hits, accounted anchor hits, emphasized high/medium/low items, confirmed functions, inferred functions, interfaces, non-functional requirements, open questions, conflicts, and exclusions. State the main analysis limitations.
