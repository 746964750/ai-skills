...
## Target-subject anchor sweep

Before classification, search case-insensitively for the declared target subject's names, its documented aliases, and any section/table/diagram scoped to it (see the scope reference). For each hit:
...
Do not treat every generic “系统” as the target subject. It is an alias only when the document defines it or the enclosing section/table/diagram makes the referent clear.
